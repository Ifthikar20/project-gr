...
│ run_count        │   │ polyline; re-checked  │   │ placed_by=system  │   │ 200 ft claims it —     │
...
- **Route run** — `POST /v1/runs/{route_id}/complete` now *also* scans the
  ...
  whose coordinates the track passed within 200 ft (61 m) is claimed automatically
...
2. **Track-verified.** The GPS track must actually pass within
   `DROP_COLLECT_RADIUS_M` (61 m = 200 ft) of the stored coordinates
...
| `outcome` | `awarded` · `already_taken` (lost the race) · `too_far` (track never provably within 200 ft) · `own_drop` |
...
200 m-accuracy fix can't prove you were within 200 ft of anything, so a claim
